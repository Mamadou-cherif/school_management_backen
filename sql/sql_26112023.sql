...
--
-- Hôte : 127.0.0.1
-- Généré le : dim. 26 nov. 2023 à 08:36
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `notes_insert` (`eleveId` INT(11), `matiereId` INT(11), `valeur` INT(11), `mois` VARCHAR(45), `isOral` INT(11), `isEcrit` INT(11), `observations` LONGTEXT, `creationUserId` INT)   BEGIN
	 INSERT INTO notes( 
		 id, 
		 eleveId,
		 matiereId,
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `notes_selectBy` (`id` INT(11), `eleveId` INT(11), `matiereId` INT(11), `valeur` INT(11), `mois` VARCHAR(45), `isOral` INT(11), `isEcrit` INT(11), `observations` LONGTEXT, `estActif` TINYINT(4), `creationDate` DATETIME, `creationUserId` INT(11), `modifDate` DATETIME, `modifUserId` INT(11))   BEGIN
 DECLARE requeteSql varchar(500); 
	 SET @requeteSql :=   
		 'SELECT
			notes.id,
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `notes_update` (`id` INT, `eleveId` INT(11), `matiereId` INT(11), `valeur` INT(11), `mois` VARCHAR(45), `isOral` INT(11), `isEcrit` INT(11), `observations` LONGTEXT, `modifDate` DATETIME, `modifUserId` INT)   BEGIN
	 UPDATE notes
		 SET 
			eleveId = eleveId, 
			matiereId = matiereId, 
...
CREATE TABLE `notes` (
  ...
  `mois` varchar(45) DEFAULT NULL COMMENT 'Les champs isOral et isEcrit vont être renseignés seulement pour les classes de niveaux supérieures',
...
--
ALTER TABLE `notes`
  ADD CONSTRAINT `fk_matieres_eleveId` FOREIGN KEY (`eleveId`) REFERENCES `eleves` (`id`) ON UPDATE CASCADE,
  ADD CONSTRAINT `fk_matieres_matiereId` FOREIGN KEY (`matiereId`) REFERENCES `matieres` (`id`) ON UPDATE CASCADE;
