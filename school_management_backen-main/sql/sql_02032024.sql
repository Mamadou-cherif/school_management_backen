...
--
-- Hôte : 127.0.0.1
-- Généré le : sam. 02 mars 2024 à 03:50
 ...
 DECLARE requeteSql varchar(500); 
	 SET @requeteSql :=   
		 'SELECT
			...
			classses.modifUserId
		 FROM classses
		LEFT JOIN `users` ON classses.enseignantId = `users`.`id`
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `eleves_getEleveOfficialClassement` (`classeId` INT, `sessionId` INT)   BEGIN
		SELECT 
			eleves.id,
			eleves.sessionId,
			eleves.classeId,
            ...
            (
				select 
					count(tickets.id)
                    from tickets
				where tickets.classeId = 11 and tickets.typeEvaluation = "Officielle" and  tickets.sessionId = sessionId
            ) as numberOfOficialEvaluation,
            
            (
				select 
                AVG(valeur)
			 from notes 
			 inner JOIN tickets on notes.ticketId = tickets.id
			 where eleves.estActif =1 and eleveId = eleves.id and tickets.sessionId = sessionId and tickets.typeEvaluation = "Officielle"
            ) as moyenneAnnuelle
		 FROM eleves
         inner JOIN classses on eleves.classeId = classses.id 
         where classses.estActif = 1 and classses.id = classeId;
END$$
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `eleves_makeClassement` (`ticketId` INT, `classeId` INT)   BEGIN
	  SELECT 
            ...
            ( select sum(valeur)
				from notes
                where 
                notes.ticketId = ticketId and 
                eleves.id = notes.eleveId and
                notes.estActif = 1
			) as totalNote,
            (
				select count(id)
                from ticket_matieres
                where ticket_matieres.tiketId = ticketId
            ) as nbMatiere,
			ROUND(( select sum(valeur)
				from notes
                where 
                notes.ticketId = ticketId and 
                eleves.id = notes.eleveId and
                ...
                from ticket_matieres
                where ticket_matieres.tiketId = ticketId
            ),2) as moyenneGenerale
			
		 FROM eleves
         where eleves.estActif = 1 and eleves.classeId = classeId
		 and eleves.id in (
			select 
				notes.eleveId
			from notes
            
            where notes.ticketId = ticketId
         )
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `eleves_makeSecondaryClassement` (`ticketId` INT, `classeId` INT)   BEGIN
	  SELECT 
			eleves.id,
			eleves.sessionId,
			eleves.classeId,
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `eleves_selectBy` (`id` INT(11), `sessionId` INT(11), `classeId` INT(11), `matricule` VARCHAR(45), `nom` VARCHAR(150), `prenoms` VARCHAR(150), `numeroTuteur1` DOUBLE, `numeroTuteur2` DOUBLE, `numeroTuteur4` DOUBLE, `statut` VARCHAR(45), `estActif` TINYINT(4), `creationDate` DATETIME, `creationUserId` INT(11), `modifDate` DATETIME, `modifUserId` INT(11))   BEGIN
 DECLARE requeteSql varchar(500); 
	 ...
	 IF sessionId IS NOT NULL THEN 
		 SET @requeteSql := CONCAT(@requeteSql, ' AND eleves.sessionId = ' ,sessionId); 
	 END IF; 
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `enseignantclasses_getClasseAffectedToEnseignant` (`enseignantId` INT)   BEGIN
	 SELECT 
			enseignantclasses.id,
			classses.libelle,
            classses.id as classeId,
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `enseignantclasses_selectAll` (`estActif` TINYINT, `debut` INT, `fin` INT)   BEGIN 
	 ...
	 Set @requeteSql := 'SELECT 
			...
			enseignantclasses.creationUserId,
			enseignantclasses.modifDate,
			enseignantclasses.modifUserId
		 FROM enseignantclasses';
 	 SET @requeteSql := CONCAT(@requeteSql,' WHERE enseignantclasses.estActif = ', estActif); 
	 IF debut IS NOT NULL THEN 
	 SET @requeteSql := CONCAT(@requeteSql, ' LIMIT ',debut, ', ',fin); 
	 END IF; 
	 PREPARE statement FROM @requeteSql;
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `enseignantclasses_selectBy` (`id` INT(11), `sessionId` INT(11), `enseignantId` INT(11), `matiereId` INT(11), `estPpricipale` INT(11), `estActif` TINYINT(4), `creationDate` DATETIME, `creationUserId` INT(11), `modifDate` DATETIME, `modifUserId` INT(11))   BEGIN
 DECLARE requeteSql varchar(500); 
	 SET @requeteSql :=   
		...
		WHERE 1';
	 IF id IS NOT NULL THEN 
		 SET @requeteSql := CONCAT(@requeteSql, ' AND enseignantclasses.id = ' ,id); 
	 END IF; 
	 IF sessionId IS NOT NULL THEN 
		 SET @requeteSql := CONCAT(@requeteSql, ' AND enseignantclasses.sessionId = ' ,sessionId); 
	 END IF; 
	 IF enseignantId IS NOT NULL THEN 
	 ...
	 END IF; 
	 IF estActif IS NOT NULL THEN 
		 SET @requeteSql := CONCAT(@requeteSql, ' AND enseignantclasses.estActif = "',estActif,'"');
	 END IF; 
	 IF creationDate IS NOT NULL THEN 
		 SET @requeteSql := CONCAT(@requeteSql, ' AND enseignantclasses.creationDate = "',creationDate,'"');
	 END IF; 
	 IF creationUserId IS NOT NULL THEN 
		 SET @requeteSql := CONCAT(@requeteSql, ' AND enseignantclasses.creationUserId = ' ,creationUserId); 
	 END IF; 
	 IF modifDate IS NOT NULL THEN 
		 SET @requeteSql := CONCAT(@requeteSql, ' AND enseignantclasses.modifDate = "',modifDate,'"');
	 END IF; 
	 IF modifUserId IS NOT NULL THEN 
		 SET @requeteSql := CONCAT(@requeteSql, ' AND enseignantclasses.modifUserId = ' ,modifUserId); 
	 END IF; 
	 PREPARE statement FROM @requeteSql;
	 EXECUTE statement;
	 DEALLOCATE PREPARE statement;
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `matieres_selectAll` (`estActif` TINYINT, `debut` INT, `fin` INT)   BEGIN 
	 ...
	 Set @requeteSql := 'SELECT 
			...
			matieres.estActif,
			matieres.creationDate,
			matieres.creationUserId,
			matieres.modifDate,
			matieres.modifUserId
		 FROM matieres';
 	 SET @requeteSql := CONCAT(@requeteSql,' WHERE matieres.estActif = ', estActif); 
	 IF debut IS NOT NULL THEN 
	 SET @requeteSql := CONCAT(@requeteSql, ' LIMIT ',debut, ', ',fin); 
	 END IF; 
	 PREPARE statement FROM @requeteSql;
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `matieres_selectBy` (`id` INT(11), `classeId` INT(11), `libelle` VARCHAR(50), `coefficient` INT(11), `estActif` TINYINT(4), `creationDate` DATETIME, `creationUserId` INT(11), `modifDate` DATETIME, `modifUserId` INT(11))   BEGIN
 DECLARE requeteSql varchar(500); 
	 SET @requeteSql :=   
		...
		WHERE 1';
	 IF id IS NOT NULL THEN 
		 SET @requeteSql := CONCAT(@requeteSql, ' AND matieres.id = ' ,id); 
	 END IF; 
	 IF classeId IS NOT NULL THEN 
		 SET @requeteSql := CONCAT(@requeteSql, ' AND matieres.classeId = ' ,classeId); 
	 END IF; 
	 IF libelle IS NOT NULL THEN 
		 SET @requeteSql := CONCAT(@requeteSql, ' AND matieres.libelle = "',libelle,'"');
	 END IF; 
	 IF coefficient IS NOT NULL THEN 
		 SET @requeteSql := CONCAT(@requeteSql, ' AND matieres.coefficient = ' ,coefficient); 
	 END IF; 
	 IF estActif IS NOT NULL THEN 
		 SET @requeteSql := CONCAT(@requeteSql, ' AND matieres.estActif = "',estActif,'"');
	 END IF; 
	 IF creationDate IS NOT NULL THEN 
		 SET @requeteSql := CONCAT(@requeteSql, ' AND matieres.creationDate = "',creationDate,'"');
	 END IF; 
	 IF creationUserId IS NOT NULL THEN 
		 SET @requeteSql := CONCAT(@requeteSql, ' AND matieres.creationUserId = ' ,creationUserId); 
	 END IF; 
	 IF modifDate IS NOT NULL THEN 
		 SET @requeteSql := CONCAT(@requeteSql, ' AND matieres.modifDate = "',modifDate,'"');
	 END IF; 
	 IF modifUserId IS NOT NULL THEN 
		 SET @requeteSql := CONCAT(@requeteSql, ' AND matieres.modifUserId = ' ,modifUserId); 
	 END IF; 
	 PREPARE statement FROM @requeteSql;
	 EXECUTE statement;
	 DEALLOCATE PREPARE statement;
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `notes_selectAll` (`estActif` TINYINT, `debut` INT, `fin` INT)   BEGIN 
	 ...
	 Set @requeteSql := 'SELECT 
			...
			notes.estActif,
			notes.creationDate,
			notes.creationUserId,
			notes.modifDate,
			notes.modifUserId
		 FROM notes';
 	 SET @requeteSql := CONCAT(@requeteSql,' WHERE notes.estActif = ', estActif); 
	 IF debut IS NOT NULL THEN 
	 SET @requeteSql := CONCAT(@requeteSql, ' LIMIT ',debut, ', ',fin); 
	 END IF; 
	 PREPARE statement FROM @requeteSql;
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `notes_selectBy` (`id` INT(11), `ticketId` INT(11), `eleveId` INT(11), `matiereId` INT(11), `valeur` INT(11), `semestre` VARCHAR(100), `mois` VARCHAR(45), `isOral` INT(11), `isEcrit` INT(11), `observations` LONGTEXT, `estActif` TINYINT(4), `creationDate` DATETIME, `creationUserId` INT(11), `modifDate` DATETIME, `modifUserId` INT(11))   BEGIN
 DECLARE requeteSql varchar(500); 
	 SET @requeteSql :=   
		...
		WHERE 1';
	 IF id IS NOT NULL THEN 
		 SET @requeteSql := CONCAT(@requeteSql, ' AND notes.id = ' ,id); 
	 END IF; 
	 IF ticketId IS NOT NULL THEN 
		 SET @requeteSql := CONCAT(@requeteSql, ' AND notes.ticketId = ' ,ticketId); 
	 END IF; 
	 IF eleveId IS NOT NULL THEN 
		 SET @requeteSql := CONCAT(@requeteSql, ' AND notes.eleveId = ' ,eleveId); 
	 END IF; 
	 IF matiereId IS NOT NULL THEN 
		 SET @requeteSql := CONCAT(@requeteSql, ' AND notes.matiereId = ' ,matiereId); 
	 END IF; 
	 IF valeur IS NOT NULL THEN 
		 SET @requeteSql := CONCAT(@requeteSql, ' AND notes.valeur = ' ,valeur); 
	 END IF; 
	 IF semestre IS NOT NULL THEN 
		 SET @requeteSql := CONCAT(@requeteSql, ' AND notes.semestre = "',semestre,'"');
	 END IF; 
	 IF mois IS NOT NULL THEN 
		 SET @requeteSql := CONCAT(@requeteSql, ' AND notes.mois = "',mois,'"');
	 END IF; 
	 IF isOral IS NOT NULL THEN 
		 SET @requeteSql := CONCAT(@requeteSql, ' AND notes.isOral = ' ,isOral); 
	 END IF; 
	 IF isEcrit IS NOT NULL THEN 
		 SET @requeteSql := CONCAT(@requeteSql, ' AND notes.isEcrit = ' ,isEcrit); 
	 END IF; 
	 IF observations IS NOT NULL THEN 
		 SET @requeteSql := CONCAT(@requeteSql, ' AND notes.observations = "',observations,'"');
	 END IF; 
	 IF estActif IS NOT NULL THEN 
		 SET @requeteSql := CONCAT(@requeteSql, ' AND notes.estActif = "',estActif,'"');
	 END IF; 
	 IF creationDate IS NOT NULL THEN 
		 SET @requeteSql := CONCAT(@requeteSql, ' AND notes.creationDate = "',creationDate,'"');
	 END IF; 
	 IF creationUserId IS NOT NULL THEN 
		 SET @requeteSql := CONCAT(@requeteSql, ' AND notes.creationUserId = ' ,creationUserId); 
	 END IF; 
	 IF modifDate IS NOT NULL THEN 
		 SET @requeteSql := CONCAT(@requeteSql, ' AND notes.modifDate = "',modifDate,'"');
	 END IF; 
	 IF modifUserId IS NOT NULL THEN 
		 SET @requeteSql := CONCAT(@requeteSql, ' AND notes.modifUserId = ' ,modifUserId); 
	 END IF; 
	 PREPARE statement FROM @requeteSql;
	 EXECUTE statement;
	 DEALLOCATE PREPARE statement;
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `payements_getStorySaleByEleve` (`eleveId` INT, `mois` VARCHAR(54))   BEGIN
	SELECT 
		 ...
		 FROM storysales
         INNER JOIN eleves  on storysales.eleveId = eleves.id
		 LEFT JOIN prestations  on storysales.prestationId = prestations.id
		 WHERE eleves.estActif= 1 AND eleves.id = eleveId
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `sessions_selectAll` (`estActif` TINYINT, `debut` INT, `fin` INT)   BEGIN 
	 ...
	 Set @requeteSql := 'SELECT 
			...
			sessions.estActif,
			sessions.creationDate,
			sessions.creationUserId,
			sessions.modifDate,
			sessions.modifUserId
		 FROM sessions';
 	 SET @requeteSql := CONCAT(@requeteSql,' WHERE sessions.estActif = ', estActif); 
	 IF debut IS NOT NULL THEN 
	 SET @requeteSql := CONCAT(@requeteSql, ' LIMIT ',debut, ', ',fin); 
	 END IF; 
	 PREPARE statement FROM @requeteSql;
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `sessions_selectBy` (`id` INT(11), `libelle` VARCHAR(45), `observations` LONGTEXT, `estActif` TINYINT(4), `creationDate` DATETIME, `creationUserId` INT(11), `modifDate` DATETIME, `modifUserId` INT(11))   BEGIN
 DECLARE requeteSql varchar(500); 
	 SET @requeteSql :=   
		...
		WHERE 1';
	 IF id IS NOT NULL THEN 
		 SET @requeteSql := CONCAT(@requeteSql, ' AND sessions.id = ' ,id); 
	 END IF; 
	 IF libelle IS NOT NULL THEN 
		 SET @requeteSql := CONCAT(@requeteSql, ' AND sessions.libelle = "',libelle,'"');
	 END IF; 
	 IF observations IS NOT NULL THEN 
		 SET @requeteSql := CONCAT(@requeteSql, ' AND sessions.observations = "',observations,'"');
	 END IF; 
	 IF estActif IS NOT NULL THEN 
		 SET @requeteSql := CONCAT(@requeteSql, ' AND sessions.estActif = "',estActif,'"');
	 END IF; 
	 IF creationDate IS NOT NULL THEN 
		 SET @requeteSql := CONCAT(@requeteSql, ' AND sessions.creationDate = "',creationDate,'"');
	 END IF; 
	 IF creationUserId IS NOT NULL THEN 
		 SET @requeteSql := CONCAT(@requeteSql, ' AND sessions.creationUserId = ' ,creationUserId); 
	 END IF; 
	 IF modifDate IS NOT NULL THEN 
		 SET @requeteSql := CONCAT(@requeteSql, ' AND sessions.modifDate = "',modifDate,'"');
	 END IF; 
	 IF modifUserId IS NOT NULL THEN 
		 SET @requeteSql := CONCAT(@requeteSql, ' AND sessions.modifUserId = ' ,modifUserId); 
	 END IF; 
	 PREPARE statement FROM @requeteSql;
	 EXECUTE statement;
	 DEALLOCATE PREPARE statement;
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `tickets_getAllTicketsForByEcoleId` (`ecoleId` INT)   BEGIN
	SELECT 
		 ...
		 FROM tickets
         INNER JOIN classses on tickets.classeId = classses.id
		 where classses.ecoleId = ecoleId and classses.estActif =1 and tickets.estActif=1
         ;
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `tickets_insert` (`sessionId` INT(11), `classeId` INT(11), `libelle` VARCHAR(50), `typeEvaluation` VARCHAR(50), `observations` LONGTEXT, `creationUserId` INT)   BEGIN
	 INSERT INTO tickets( 
		 id, 
		 sessionId,
		 classeId,
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `tickets_selectAll` (`estActif` TINYINT, `debut` INT, `fin` INT)   BEGIN 
	 ...
	 Set @requeteSql := 'SELECT 
			...
			tickets.creationUserId,
			tickets.modifDate,
			tickets.modifUserId
		 FROM tickets';
 	 SET @requeteSql := CONCAT(@requeteSql,' WHERE tickets.estActif = ', estActif); 
	 IF debut IS NOT NULL THEN 
	 SET @requeteSql := CONCAT(@requeteSql, ' LIMIT ',debut, ', ',fin); 
	 END IF; 
	 PREPARE statement FROM @requeteSql;
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `tickets_selectBy` (`id` INT(11), `sessionId` INT(11), `classeId` INT(11), `libelle` VARCHAR(50), `typeEvaluation` VARCHAR(50), `observations` LONGTEXT, `estActif` TINYINT(4), `creationDate` DATETIME, `creationUserId` INT(11), `modifDate` DATETIME, `modifUserId` INT(11))   BEGIN
 DECLARE requeteSql varchar(500); 
	 SET @requeteSql :=   
		 'SELECT
			tickets.id,
			...
			tickets.modifUserId
		 FROM tickets
         INNER JOIN sessions on tickets.sessionId = sessions.id
         
		WHERE 1';
	 IF id IS NOT NULL THEN 
		 SET @requeteSql := CONCAT(@requeteSql, ' AND tickets.id = ' ,id); 
	 END IF; 
	 IF sessionId IS NOT NULL THEN 
		 SET @requeteSql := CONCAT(@requeteSql, ' AND tickets.sessionId = ' ,sessionId); 
	 END IF; 
	 IF classeId IS NOT NULL THEN 
		 SET @requeteSql := CONCAT(@requeteSql, ' AND tickets.classeId = ' ,classeId); 
	 END IF; 
	 IF libelle IS NOT NULL THEN 
		 SET @requeteSql := CONCAT(@requeteSql, ' AND tickets.libelle = "',libelle,'"');
	 END IF; 
     IF typeEvaluation IS NOT NULL THEN 
		 SET @requeteSql := CONCAT(@requeteSql, ' AND tickets.typeEvaluation = "',typeEvaluation,'"');
	 END IF; 
	 IF observations IS NOT NULL THEN 
		 SET @requeteSql := CONCAT(@requeteSql, ' AND tickets.observations = "',observations,'"');
	 END IF; 
	 IF estActif IS NOT NULL THEN 
		 SET @requeteSql := CONCAT(@requeteSql, ' AND tickets.estActif = "',estActif,'"');
	 END IF; 
	 IF creationDate IS NOT NULL THEN 
		 SET @requeteSql := CONCAT(@requeteSql, ' AND tickets.creationDate = "',creationDate,'"');
	 END IF; 
	 IF creationUserId IS NOT NULL THEN 
		 SET @requeteSql := CONCAT(@requeteSql, ' AND tickets.creationUserId = ' ,creationUserId); 
	 END IF; 
	 IF modifDate IS NOT NULL THEN 
		 SET @requeteSql := CONCAT(@requeteSql, ' AND tickets.modifDate = "',modifDate,'"');
	 END IF; 
	 IF modifUserId IS NOT NULL THEN 
		 SET @requeteSql := CONCAT(@requeteSql, ' AND tickets.modifUserId = ' ,modifUserId); 
	 END IF; 
	 PREPARE statement FROM @requeteSql;
	 EXECUTE statement;
	 DEALLOCATE PREPARE statement;
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `tickets_update` (`id` INT, `sessionId` INT(11), `classeId` INT(11), `libelle` VARCHAR(50), `typeEvaluation` VARCHAR(50), `observations` LONGTEXT, `modifDate` DATETIME, `modifUserId` INT)   BEGIN
	 UPDATE tickets
		 SET 
			sessionId = sessionId, 
			classeId = classeId, 
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `ticket_matieres_selectAll` (`estActif` TINYINT, `debut` INT, `fin` INT)   BEGIN 
	 ...
	 Set @requeteSql := 'SELECT 
			ticket_matieres.id,
			ticket_matieres.tiketId,
			ticket_matieres.matiereId,
			ticket_matieres.estActif,
			ticket_matieres.creationDate,
			ticket_matieres.creationUserId,
			ticket_matieres.modifDate,
			ticket_matieres.modifUserId
		 FROM ticket_matieres';
 	 SET @requeteSql := CONCAT(@requeteSql,' WHERE ticket_matieres.estActif = ', estActif); 
	 IF debut IS NOT NULL THEN 
	 SET @requeteSql := CONCAT(@requeteSql, ' LIMIT ',debut, ', ',fin); 
	 END IF; 
	 PREPARE statement FROM @requeteSql;
 ...
 DECLARE requeteSql varchar(500); 
	 SET @requeteSql :=   
		 'SELECT
			...
			ticket_matieres.modifUserId
		 FROM ticket_matieres
         INNER JOIN matieres on ticket_matieres.matiereId = matieres.id
		WHERE 1';
	 IF id IS NOT NULL THEN 
		 SET @requeteSql := CONCAT(@requeteSql, ' AND ticket_matieres.id = ' ,id); 
	 END IF; 
	 IF tiketId IS NOT NULL THEN 
		 SET @requeteSql := CONCAT(@requeteSql, ' AND ticket_matieres.tiketId = ' ,tiketId); 
	 END IF; 
	 IF matiereId IS NOT NULL THEN 
		 SET @requeteSql := CONCAT(@requeteSql, ' AND ticket_matieres.matiereId = ' ,matiereId); 
	 END IF; 
	 IF estActif IS NOT NULL THEN 
		 SET @requeteSql := CONCAT(@requeteSql, ' AND ticket_matieres.estActif = "',estActif,'"');
	 END IF; 
	 IF creationDate IS NOT NULL THEN 
		 SET @requeteSql := CONCAT(@requeteSql, ' AND ticket_matieres.creationDate = "',creationDate,'"');
	 END IF; 
	 IF creationUserId IS NOT NULL THEN 
		 SET @requeteSql := CONCAT(@requeteSql, ' AND ticket_matieres.creationUserId = ' ,creationUserId); 
	 END IF; 
	 IF modifDate IS NOT NULL THEN 
		 SET @requeteSql := CONCAT(@requeteSql, ' AND ticket_matieres.modifDate = "',modifDate,'"');
	 END IF; 
	 IF modifUserId IS NOT NULL THEN 
		 SET @requeteSql := CONCAT(@requeteSql, ' AND ticket_matieres.modifUserId = ' ,modifUserId); 
	 END IF; 
	 PREPARE statement FROM @requeteSql;
	 EXECUTE statement;
	 DEALLOCATE PREPARE statement;
...
INSERT INTO `eleves` (`id`, `sessionId`, `classeId`, `matricule`, `nom`, `prenoms`, `numeroTuteur1`, `numeroTuteur2`, `numeroTuteur4`, `statut`, `estActif`, `creationDate`, `creationUserId`, `modifDate`, `modifUserId`) VALUES
(85, 1, 1, 'AC0000', 'Camara', 'Aboulaye', 621134573, 0, 0, '', 0, '2023-10-28 15:36:01', 5, '2023-10-28 16:57:15', 5),
(86, 1, 1, 'TS0001', 'Souleymane', 'Thierno', 621134575, 0, 0, '', 0, '2023-10-28 15:36:01', 5, '2023-10-28 17:01:17', 5),
(87, 1, 1, 'MS0002', 'Sylla', 'mamadou', 621123456, 0, 0, '', 0, '2023-10-28 15:36:51', 5, '2023-10-28 16:58:38', 5),
(88, 1, 1, 'MS0003', 'Sylla', 'mamadou', 621123456, 0, 0, '', 1, '2023-10-28 15:37:04', 5, '2023-10-28 15:37:04', 5),
(89, 1, 1, 'MS0004', 'Sylla', 'mamadou', 621123456, 0, 0, '', 0, '2023-10-28 15:37:15', 5, '2023-10-28 16:57:24', 5),
(90, 1, 1, 'MS0005', 'Sylla', 'mamadou', 621123456, 0, 0, '', 0, '2023-10-28 15:38:58', 5, '2023-11-08 21:54:28', 5),
(91, 1, 2, 'FB0000', 'Bangoura', 'Fatoumata', 621121213, 621154309, 0, '', 1, '2023-11-05 22:21:43', 5, '2023-11-05 22:21:43', 5),
(92, 1, 2, 'TSS0001', 'Sylla', 'Thierno Souleymane', 629334509, 610309876, 0, '', 1, '2023-11-05 22:21:43', 5, '2023-11-05 22:21:43', 5),
(93, 1, 2, 'AY0002', 'Yansané', 'Abdoulaye', 654333333, 621106376, 0, '', 1, '2023-11-05 22:21:43', 5, '2023-11-05 22:21:43', 5),
(94, 1, 2, 'SAL0003', 'Lankono', 'Sekou Ahmed', 657101234, 627132456, 0, '', 1, '2023-11-05 22:21:43', 5, '2023-11-05 22:21:43', 5),
(95, 1, 2, 'FLB0004', 'Bangoura', 'Fodé lansana', 621134567, 655780987, 0, '', 1, '2023-11-05 22:21:43', 5, '2023-11-05 22:21:43', 5),
(96, 1, 2, 'FS0005', 'sow', 'Fatoumata', 623445678, 620212325, 0, '', 1, '2023-11-05 22:21:43', 5, '2023-11-05 22:21:43', 5),
(97, 1, 2, 'AB0006', 'Bangoura', 'Amara', 620150233, 0, 0, '', 1, '2023-11-05 22:21:43', 5, '2023-11-05 22:21:43', 5),
(98, 1, 2, 'ISB0007', 'Barry', 'Ibrahima Sory', 664256739, 620101010, 0, '', 1, '2023-11-05 22:21:43', 5, '2023-11-05 22:21:43', 5),
(99, 1, 2, 'AT0008', 'Tolno', 'Augustin', 629304418, 610214367, 0, '', 1, '2023-11-05 22:21:43', 5, '2023-11-05 22:21:43', 5),
(100, 1, 2, 'FBS0009', 'Sow', 'Fatoumata Bindy', 624567687, 654331008, 0, '', 1, '2023-11-05 22:21:43', 5, '2023-11-05 22:21:43', 5),
(101, 1, 2, 'TMS0010', 'Sylla', 'Thierno Mamadou', 620109876, 625331002, 0, '', 1, '2023-11-05 22:21:43', 5, '2023-11-05 22:21:43', 5),
(102, 1, 2, 'MB0011', 'Bah', 'Mamadou', 627000000, 623540016, 0, '', 1, '2023-11-05 22:21:43', 5, '2023-11-05 22:21:43', 5),
(103, 1, 2, 'FB0012', 'Bakayoko', 'Fodé', 628094517, 7621123465, 0, '', 1, '2023-11-05 22:21:43', 5, '2023-11-05 22:21:43', 5),
(104, 1, 6, 'AC0000', 'Camara', 'Abdoulaye', 620303030, 0, 0, '', 1, '2023-11-11 02:17:29', 5, '2023-11-11 02:17:29', 5),
(105, 1, 1, 'FC0006', 'conté', 'Fatoumata', 621330015, 0, 0, '', 1, '2023-11-11 22:53:42', 5, '2023-11-11 22:53:42', 5),
(106, 1, 1, 'AB0007', 'Barrty', 'Abdoulay', 620104365, 0, 0, '', 1, '2023-11-11 22:53:42', 5, '2023-11-16 18:08:50', 5),
(107, 1, 3, 'SN0000', 'nanahara', 'Souha', 623540016, 0, 0, '', 1, '2023-11-12 16:54:50', 5, '2023-11-12 16:54:50', 5),
(108, 1, 3, 'SD0001', 'Diallo', 'Saliou', 655101191, 0, 0, '', 1, '2023-11-12 16:54:50', 5, '2023-11-12 16:54:50', 5),
(109, 1, 4, 'MFT0000', 'Touré', 'Mohamed Fassya', 610202020, 0, 0, '', 1, '2023-11-12 16:59:37', 5, '2023-11-12 16:59:37', 5),
(110, 1, 4, 'AS0001', 'Sembene', 'Abdoulaye', 654303030, 0, 0, '', 1, '2023-11-12 16:59:37', 5, '2023-11-12 16:59:37', 5),
(111, 1, 11, 'AT0000', 'Touré', 'Algassimou', 621141502, 0, 0, '', 1, '2024-02-18 15:13:10', 5, '2024-02-18 15:13:10', 5),
(112, 1, 11, 'OT0001', 'Tounkara', 'Ousmane', 628121725, 0, 0, '', 1, '2024-02-18 15:13:10', 5, '2024-02-18 15:13:10', 5),
(113, 1, 11, 'AFS0002', 'Sylla', 'Abdoulaye Fodé', 610101215, 0, 0, '', 1, '2024-02-18 15:13:10', 5, '2024-02-18 15:13:10', 5);

-- --------------------------------------------------------

--
-- Structure de la table `enseignantclasses`
--
...
--
-- Déchargement des données de la table `enseignantclasses`
--
...
(5, 1, 34, 25, 0, 1, '2024-02-05 22:09:11', 5, '2024-02-05 22:09:11', 5),
(6, 1, 35, 26, 1, 1, '2024-02-05 22:10:20', 5, '2024-02-05 22:10:20', 5),
(7, 1, 36, 27, 0, 1, '2024-02-05 22:11:02', 5, '2024-02-05 22:11:02', 5),
(8, 1, 37, 28, 0, 1, '2024-02-05 22:11:29', 5, '2024-02-05 22:11:29', 5),
(9, 1, 38, 29, 0, 1, '2024-02-05 22:12:42', 5, '2024-02-05 22:12:42', 5),
(10, 1, 39, 30, 0, 1, '2024-02-05 22:13:31', 5, '2024-02-05 22:13:31', 5);
...
(19, 'Professeurs chargés de cours', '', 1, '2024-02-05 20:45:43', 5, '2024-02-05 20:48:18', 2024),
(20, 'Direction', '', 1, '2024-02-29 22:18:57', 5, '2024-02-29 22:18:57', 5);
...
INSERT INTO `matieres` (`id`, `classeId`, `libelle`, `coefficient`, `estActif`, `creationDate`, `creationUserId`, `modifDate`, `modifUserId`) VALUES
(6, 1, 'Physique', 1, 1, '2023-11-26 07:15:32', 5, '2023-11-26 07:15:32', 5),
(7, 1, 'Chimie', 1, 1, '2023-11-26 07:16:09', 5, '2023-11-26 07:33:52', 5),
(8, 1, 'Mathématiques', 2, 1, '2023-11-26 07:16:22', 5, '2023-11-26 07:16:22', 5),
(9, 1, 'Anglais', 1, 1, '2023-11-26 07:16:47', 5, '2023-11-26 07:16:47', 5),
(10, 1, 'Biologie', 1, 1, '2023-11-26 07:18:21', 5, '2023-11-26 07:18:21', 5),
(11, 2, 'Dictée & questions', 1, 1, '2023-12-16 16:42:26', 5, '2023-12-16 16:42:26', 5),
(12, 2, 'Redaction', 1, 1, '2023-12-16 16:42:38', 5, '2023-12-16 16:42:38', 5),
(13, 2, 'Sciences d\'observations', 1, 1, '2023-12-16 16:42:47', 5, '2023-12-16 16:42:47', 5),
(14, 2, 'Calcul écrit', 1, 1, '2023-12-16 16:43:01', 5, '2023-12-16 16:43:01', 5),
(15, 2, 'Histoire', 1, 1, '2023-12-16 16:43:09', 5, '2023-12-16 16:43:09', 5),
(16, 2, 'Geographie', 1, 1, '2023-12-16 16:43:21', 5, '2023-12-16 16:43:21', 5),
(17, 2, 'Recitation & chant', 1, 1, '2023-12-16 16:43:46', 5, '2023-12-16 16:43:46', 5),
(18, 10, 'Chimie', 2, 1, '2024-02-01 22:38:06', 5, '2024-02-01 22:38:06', 5),
(19, 10, 'Mathématique', 2, 1, '2024-02-01 22:38:18', 5, '2024-02-01 22:38:18', 5),
(20, 10, 'Anglais', 1, 1, '2024-02-01 22:38:37', 5, '2024-02-01 22:38:37', 5),
...
(102, 'classe', 'Classes', '', '/eleve/listeClasse', 0, 6, 'Prive', '\nsecteur.png', 1, '2023-11-23 23:54:35', 5, '2023-11-23 23:54:35', 5),
(103, 'note_sm', 'Note', '', '/note/note_sm', 0, 0, 'Prive', 'sousmenuservices.png', 1, '2023-12-16 16:22:00', 5, '2023-12-16 16:22:00', 5),
(104, 'resultats', 'Resultats Evaluations', '', '/note/resultat', 0, 10, 'Prive', 'sousmenuprogrammeprioritaire.png', 1, '2023-12-17 00:30:29', 5, '2023-12-17 00:30:29', 5),
(105, 'ticket_sm', 'Tickets', '', '/ticket/consultationTicket', 0, 15, 'Prive', 'a.jpeg', 1, '2024-02-29 22:20:28', 5, '2024-02-29 22:20:28', 5);
...
CREATE TABLE `notes` (
  ...
  `isOral` int(11) DEFAULT NULL,
  `isEcrit` int(11) DEFAULT NULL,
...
INSERT INTO `notes` (`id`, `ticketId`, `eleveId`, `matiereId`, `valeur`, `semestre`, `mois`, `isOral`, `isEcrit`, `observations`, `estActif`, `creationDate`, `creationUserId`, `modifDate`, `modifUserId`) VALUES
(636, 8, 88, 6, 5, '', NULL, 0, 0, '', 1, '2023-12-18 18:56:43', 5, '2023-12-18 18:56:43', 5),
(637, 8, 88, 7, 3, '', NULL, 0, 0, '', 1, '2023-12-18 18:56:43', 5, '2023-12-18 18:56:43', 5),
(638, 8, 88, 8, 4, '', NULL, 0, 0, '', 1, '2023-12-18 18:56:43', 5, '2023-12-18 18:56:43', 5),
(639, 8, 88, 9, 9, '', NULL, 0, 0, '', 1, '2023-12-18 18:56:43', 5, '2023-12-18 18:56:43', 5),
(640, 8, 88, 10, 7, '', NULL, 0, 0, '', 1, '2023-12-18 18:56:43', 5, '2023-12-18 18:56:43', 5),
(641, 8, 105, 6, 7, '', NULL, 0, 0, '', 1, '2023-12-18 18:56:43', 5, '2023-12-18 18:56:43', 5),
(642, 8, 105, 7, 8, '', NULL, 0, 0, '', 1, '2023-12-18 18:56:43', 5, '2023-12-18 18:56:43', 5),
(643, 8, 105, 8, 9, '', NULL, 0, 0, '', 1, '2023-12-18 18:56:43', 5, '2023-12-18 18:56:43', 5),
(644, 8, 105, 9, 10, '', NULL, 0, 0, '', 1, '2023-12-18 18:56:43', 5, '2023-12-18 18:56:43', 5),
(645, 8, 105, 10, 10, '', NULL, 0, 0, '', 1, '2023-12-18 18:56:43', 5, '2023-12-18 18:56:43', 5),
(646, 8, 106, 6, 10, '', NULL, 0, 0, '', 1, '2023-12-18 18:56:43', 5, '2023-12-18 18:56:43', 5),
(647, 8, 106, 7, 6, '', NULL, 0, 0, '', 1, '2023-12-18 18:56:43', 5, '2023-12-18 18:56:43', 5),
(648, 8, 106, 8, 2, '', NULL, 0, 0, '', 1, '2023-12-18 18:56:43', 5, '2023-12-18 18:56:43', 5),
(649, 8, 106, 9, 1, '', NULL, 0, 0, '', 1, '2023-12-18 18:56:43', 5, '2023-12-18 18:56:43', 5),
(650, 8, 106, 10, 2, '', NULL, 0, 0, '', 1, '2023-12-18 18:56:43', 5, '2023-12-18 18:56:43', 5),
(651, 6, 91, 11, 3, 'Semestre 1', '', 0, 0, '', 1, '2023-12-18 23:38:49', 5, '2023-12-18 23:38:49', 5),
(652, 6, 91, 12, 1, 'Semestre 1', '', 0, 0, '', 1, '2023-12-18 23:38:49', 5, '2023-12-18 23:38:49', 5),
(653, 6, 91, 13, 5, 'Semestre 1', '', 0, 0, '', 1, '2023-12-18 23:38:49', 5, '2023-12-18 23:38:49', 5),
(654, 6, 91, 14, 9, 'Semestre 1', '', 0, 0, '', 1, '2023-12-18 23:38:49', 5, '2023-12-18 23:38:49', 5),
(655, 6, 91, 15, 10, 'Semestre 1', '', 0, 0, '', 1, '2023-12-18 23:38:49', 5, '2023-12-18 23:38:49', 5),
(656, 6, 91, 16, 6, 'Semestre 1', '', 0, 0, '', 1, '2023-12-18 23:38:49', 5, '2023-12-18 23:38:49', 5),
(657, 6, 91, 17, 8, 'Semestre 1', '', 0, 0, '', 1, '2023-12-18 23:38:49', 5, '2023-12-18 23:38:49', 5),
(658, 6, 92, 11, 10, 'Semestre 1', '', 0, 0, '', 1, '2023-12-18 23:38:49', 5, '2023-12-18 23:38:49', 5),
(659, 6, 92, 12, 1, 'Semestre 1', '', 0, 0, '', 1, '2023-12-18 23:38:49', 5, '2023-12-18 23:38:49', 5),
(660, 6, 92, 13, 4, 'Semestre 1', '', 0, 0, '', 1, '2023-12-18 23:38:49', 5, '2023-12-18 23:38:49', 5),
(661, 6, 92, 14, 8, 'Semestre 1', '', 0, 0, '', 1, '2023-12-18 23:38:49', 5, '2023-12-18 23:38:49', 5),
(662, 6, 92, 15, 10, 'Semestre 1', '', 0, 0, '', 1, '2023-12-18 23:38:49', 5, '2023-12-18 23:38:49', 5),
(663, 6, 92, 16, 5, 'Semestre 1', '', 0, 0, '', 1, '2023-12-18 23:38:49', 5, '2023-12-18 23:38:49', 5),
(664, 6, 92, 17, 10, 'Semestre 1', '', 0, 0, '', 1, '2023-12-18 23:38:49', 5, '2023-12-18 23:38:49', 5),
(665, 6, 93, 11, 3, 'Semestre 1', '', 0, 0, '', 1, '2023-12-18 23:38:49', 5, '2023-12-18 23:38:49', 5),
(666, 6, 93, 12, 2, 'Semestre 1', '', 0, 0, '', 1, '2023-12-18 23:38:49', 5, '2023-12-18 23:38:49', 5),
(667, 6, 93, 13, 5, 'Semestre 1', '', 0, 0, '', 1, '2023-12-18 23:38:49', 5, '2023-12-18 23:38:49', 5),
(668, 6, 93, 14, 8, 'Semestre 1', '', 0, 0, '', 1, '2023-12-18 23:38:49', 5, '2023-12-18 23:38:49', 5),
(669, 6, 93, 15, 9, 'Semestre 1', '', 0, 0, '', 1, '2023-12-18 23:38:49', 5, '2023-12-18 23:38:49', 5),
(670, 6, 93, 16, 5, 'Semestre 1', '', 0, 0, '', 1, '2023-12-18 23:38:49', 5, '2023-12-18 23:38:49', 5),
(671, 6, 93, 17, 3, 'Semestre 1', '', 0, 0, '', 1, '2023-12-18 23:38:49', 5, '2023-12-18 23:38:49', 5),
(672, 6, 94, 11, 3, 'Semestre 1', '', 0, 0, '', 1, '2023-12-18 23:38:49', 5, '2023-12-18 23:38:49', 5),
(673, 6, 94, 12, 2, 'Semestre 1', '', 0, 0, '', 1, '2023-12-18 23:38:49', 5, '2023-12-18 23:38:49', 5),
(674, 6, 94, 13, 5, 'Semestre 1', '', 0, 0, '', 1, '2023-12-18 23:38:49', 5, '2023-12-18 23:38:49', 5),
(675, 6, 94, 14, 8, 'Semestre 1', '', 0, 0, '', 1, '2023-12-18 23:38:49', 5, '2023-12-18 23:38:49', 5),
(676, 6, 94, 15, 9, 'Semestre 1', '', 0, 0, '', 1, '2023-12-18 23:38:49', 5, '2023-12-18 23:38:49', 5),
(677, 6, 94, 16, 9, 'Semestre 1', '', 0, 0, '', 1, '2023-12-18 23:38:49', 5, '2023-12-18 23:38:49', 5),
(678, 6, 94, 17, 3, 'Semestre 1', '', 0, 0, '', 1, '2023-12-18 23:38:49', 5, '2023-12-18 23:38:49', 5),
(679, 6, 95, 11, 1, 'Semestre 1', '', 0, 0, '', 1, '2023-12-18 23:38:49', 5, '2023-12-18 23:38:49', 5),
(680, 6, 95, 12, 8, 'Semestre 1', '', 0, 0, '', 1, '2023-12-18 23:38:49', 5, '2023-12-18 23:38:49', 5),
(681, 6, 95, 13, 7, 'Semestre 1', '', 0, 0, '', 1, '2023-12-18 23:38:49', 5, '2023-12-18 23:38:49', 5),
(682, 6, 95, 14, 4, 'Semestre 1', '', 0, 0, '', 1, '2023-12-18 23:38:49', 5, '2023-12-18 23:38:49', 5),
(683, 6, 95, 15, 7, 'Semestre 1', '', 0, 0, '', 1, '2023-12-18 23:38:49', 5, '2023-12-18 23:38:49', 5),
(684, 6, 95, 16, 1, 'Semestre 1', '', 0, 0, '', 1, '2023-12-18 23:38:49', 5, '2023-12-18 23:38:49', 5),
(685, 6, 95, 17, 9, 'Semestre 1', '', 0, 0, '', 1, '2023-12-18 23:38:49', 5, '2023-12-18 23:38:49', 5),
(686, 6, 96, 11, 3, 'Semestre 1', '', 0, 0, '', 1, '2023-12-18 23:38:49', 5, '2023-12-18 23:38:49', 5),
(687, 6, 96, 12, 4, 'Semestre 1', '', 0, 0, '', 1, '2023-12-18 23:38:49', 5, '2023-12-18 23:38:49', 5),
(688, 6, 96, 13, 5, 'Semestre 1', '', 0, 0, '', 1, '2023-12-18 23:38:49', 5, '2023-12-18 23:38:49', 5),
(689, 6, 96, 14, 6, 'Semestre 1', '', 0, 0, '', 1, '2023-12-18 23:38:49', 5, '2023-12-18 23:38:49', 5),
(690, 6, 96, 15, 8, 'Semestre 1', '', 0, 0, '', 1, '2023-12-18 23:38:49', 5, '2023-12-18 23:38:49', 5),
(691, 6, 96, 16, 3, 'Semestre 1', '', 0, 0, '', 1, '2023-12-18 23:38:49', 5, '2023-12-18 23:38:49', 5),
(692, 6, 96, 17, 2, 'Semestre 1', '', 0, 0, '', 1, '2023-12-18 23:38:49', 5, '2023-12-18 23:38:49', 5),
(693, 6, 97, 11, 4, 'Semestre 1', '', 0, 0, '', 1, '2023-12-18 23:38:49', 5, '2023-12-18 23:38:49', 5),
(694, 6, 97, 12, 6, 'Semestre 1', '', 0, 0, '', 1, '2023-12-18 23:38:49', 5, '2023-12-18 23:38:49', 5),
(695, 6, 97, 13, 7, 'Semestre 1', '', 0, 0, '', 1, '2023-12-18 23:38:49', 5, '2023-12-18 23:38:49', 5),
(696, 6, 97, 14, 8, 'Semestre 1', '', 0, 0, '', 1, '2023-12-18 23:38:49', 5, '2023-12-18 23:38:49', 5),
(697, 6, 97, 15, 2, 'Semestre 1', '', 0, 0, '', 1, '2023-12-18 23:38:49', 5, '2023-12-18 23:38:49', 5),
(698, 6, 97, 16, 1, 'Semestre 1', '', 0, 0, '', 1, '2023-12-18 23:38:49', 5, '2023-12-18 23:38:49', 5),
(699, 6, 97, 17, 10, 'Semestre 1', '', 0, 0, '', 1, '2023-12-18 23:38:49', 5, '2023-12-18 23:38:49', 5),
(700, 6, 98, 11, 1, 'Semestre 1', '', 0, 0, '', 1, '2023-12-18 23:38:49', 5, '2023-12-18 23:38:49', 5),
(701, 6, 98, 12, 9, 'Semestre 1', '', 0, 0, '', 1, '2023-12-18 23:38:49', 5, '2023-12-18 23:38:49', 5),
(702, 6, 98, 13, 3, 'Semestre 1', '', 0, 0, '', 1, '2023-12-18 23:38:49', 5, '2023-12-18 23:38:49', 5),
(703, 6, 98, 14, 10, 'Semestre 1', '', 0, 0, '', 1, '2023-12-18 23:38:49', 5, '2023-12-18 23:38:49', 5),
(704, 6, 98, 15, 9, 'Semestre 1', '', 0, 0, '', 1, '2023-12-18 23:38:49', 5, '2023-12-18 23:38:49', 5),
(705, 6, 98, 16, 2, 'Semestre 1', '', 0, 0, '', 1, '2023-12-18 23:38:49', 5, '2023-12-18 23:38:49', 5),
(706, 6, 98, 17, 5, 'Semestre 1', '', 0, 0, '', 1, '2023-12-18 23:38:49', 5, '2023-12-18 23:38:49', 5),
(707, 6, 99, 11, 3, 'Semestre 1', '', 0, 0, '', 1, '2023-12-18 23:38:49', 5, '2023-12-18 23:38:49', 5),
(708, 6, 99, 12, 5, 'Semestre 1', '', 0, 0, '', 1, '2023-12-18 23:38:49', 5, '2023-12-18 23:38:49', 5),
(709, 6, 99, 13, 7, 'Semestre 1', '', 0, 0, '', 1, '2023-12-18 23:38:49', 5, '2023-12-18 23:38:49', 5),
(710, 6, 99, 14, 4, 'Semestre 1', '', 0, 0, '', 1, '2023-12-18 23:38:49', 5, '2023-12-18 23:38:49', 5),
(711, 6, 99, 15, 3, 'Semestre 1', '', 0, 0, '', 1, '2023-12-18 23:38:49', 5, '2023-12-18 23:38:49', 5),
(712, 6, 99, 16, 3, 'Semestre 1', '', 0, 0, '', 1, '2023-12-18 23:38:49', 5, '2023-12-18 23:38:49', 5),
(713, 6, 99, 17, 6, 'Semestre 1', '', 0, 0, '', 1, '2023-12-18 23:38:49', 5, '2023-12-18 23:38:49', 5),
(714, 6, 100, 11, 1, 'Semestre 1', '', 0, 0, '', 1, '2023-12-18 23:38:49', 5, '2023-12-18 23:38:49', 5),
(715, 6, 100, 12, 2, 'Semestre 1', '', 0, 0, '', 1, '2023-12-18 23:38:49', 5, '2023-12-18 23:38:49', 5),
(716, 6, 100, 13, 7, 'Semestre 1', '', 0, 0, '', 1, '2023-12-18 23:38:49', 5, '2023-12-18 23:38:49', 5),
(717, 6, 100, 14, 3, 'Semestre 1', '', 0, 0, '', 1, '2023-12-18 23:38:49', 5, '2023-12-18 23:38:49', 5),
(718, 6, 100, 15, 2, 'Semestre 1', '', 0, 0, '', 1, '2023-12-18 23:38:49', 5, '2023-12-18 23:38:49', 5),
(719, 6, 100, 16, 2, 'Semestre 1', '', 0, 0, '', 1, '2023-12-18 23:38:49', 5, '2023-12-18 23:38:49', 5),
(720, 6, 100, 17, 10, 'Semestre 1', '', 0, 0, '', 1, '2023-12-18 23:38:49', 5, '2023-12-18 23:38:49', 5),
(721, 6, 101, 11, 1, 'Semestre 1', '', 0, 0, '', 1, '2023-12-18 23:38:49', 5, '2023-12-18 23:38:49', 5),
(722, 6, 101, 12, 9, 'Semestre 1', '', 0, 0, '', 1, '2023-12-18 23:38:49', 5, '2023-12-18 23:38:49', 5),
(723, 6, 101, 13, 7, 'Semestre 1', '', 0, 0, '', 1, '2023-12-18 23:38:49', 5, '2023-12-18 23:38:49', 5),
(724, 6, 101, 14, 3, 'Semestre 1', '', 0, 0, '', 1, '2023-12-18 23:38:49', 5, '2023-12-18 23:38:49', 5),
(725, 6, 101, 15, 2, 'Semestre 1', '', 0, 0, '', 1, '2023-12-18 23:38:49', 5, '2023-12-18 23:38:49', 5),
(726, 6, 101, 16, 2, 'Semestre 1', '', 0, 0, '', 1, '2023-12-18 23:38:49', 5, '2023-12-18 23:38:49', 5),
(727, 6, 101, 17, 10, 'Semestre 1', '', 0, 0, '', 1, '2023-12-18 23:38:49', 5, '2023-12-18 23:38:49', 5),
(728, 6, 102, 11, 7, 'Semestre 1', '', 0, 0, '', 1, '2023-12-18 23:38:49', 5, '2023-12-18 23:38:49', 5),
(729, 6, 102, 12, 1, 'Semestre 1', '', 0, 0, '', 1, '2023-12-18 23:38:49', 5, '2023-12-18 23:38:49', 5),
(730, 6, 102, 13, 8, 'Semestre 1', '', 0, 0, '', 1, '2023-12-18 23:38:49', 5, '2023-12-18 23:38:49', 5),
(731, 6, 102, 14, 2, 'Semestre 1', '', 0, 0, '', 1, '2023-12-18 23:38:49', 5, '2023-12-18 23:38:49', 5),
(732, 6, 102, 15, 2, 'Semestre 1', '', 0, 0, '', 1, '2023-12-18 23:38:49', 5, '2023-12-18 23:38:49', 5),
(733, 6, 102, 16, 2, 'Semestre 1', '', 0, 0, '', 1, '2023-12-18 23:38:49', 5, '2023-12-18 23:38:49', 5),
(734, 6, 102, 17, 3, 'Semestre 1', '', 0, 0, '', 1, '2023-12-18 23:38:49', 5, '2023-12-18 23:38:49', 5),
(735, 6, 103, 11, 3, 'Semestre 1', '', 0, 0, '', 1, '2023-12-18 23:38:49', 5, '2023-12-18 23:38:49', 5),
(736, 6, 103, 12, 6, 'Semestre 1', '', 0, 0, '', 1, '2023-12-18 23:38:49', 5, '2023-12-18 23:38:49', 5),
(737, 6, 103, 13, 7, 'Semestre 1', '', 0, 0, '', 1, '2023-12-18 23:38:49', 5, '2023-12-18 23:38:49', 5),
(738, 6, 103, 14, 4, 'Semestre 1', '', 0, 0, '', 1, '2023-12-18 23:38:49', 5, '2023-12-18 23:38:49', 5),
(739, 6, 103, 15, 8, 'Semestre 1', '', 0, 0, '', 1, '2023-12-18 23:38:49', 5, '2023-12-18 23:38:49', 5),
(740, 6, 103, 16, 5, 'Semestre 1', '', 0, 0, '', 1, '2023-12-18 23:38:49', 5, '2023-12-18 23:38:49', 5),
(741, 6, 103, 17, 4, 'Semestre 1', '', 0, 0, '', 1, '2023-12-18 23:38:49', 5, '2023-12-18 23:38:49', 5),
(745, 11, 111, 25, NULL, 'Semestre 1', '', 10, 20, '', 1, '2024-02-25 01:32:29', 34, '2024-02-25 01:32:29', 34),
(746, 11, 112, 25, NULL, 'Semestre 1', '', 11, 13, '', 1, '2024-02-25 01:32:29', 34, '2024-02-25 01:32:29', 34),
(747, 11, 113, 25, NULL, 'Semestre 1', '', 9, 17, '', 1, '2024-02-25 01:32:29', 34, '2024-02-25 01:32:29', 34),
(748, 7, 91, 11, 3, '', NULL, 0, 0, '', 1, '2024-03-02 02:12:17', 5, '2024-03-02 02:12:17', 5),
(749, 7, 91, 12, 8, '', NULL, 0, 0, '', 1, '2024-03-02 02:12:17', 5, '2024-03-02 02:12:17', 5),
(750, 7, 91, 13, 8, '', NULL, 0, 0, '', 1, '2024-03-02 02:12:17', 5, '2024-03-02 02:12:17', 5),
(751, 7, 92, 11, 0, '', NULL, 0, 0, '', 1, '2024-03-02 02:12:17', 5, '2024-03-02 02:12:17', 5),
(752, 7, 92, 12, 0, '', NULL, 0, 0, '', 1, '2024-03-02 02:12:17', 5, '2024-03-02 02:12:17', 5),
(753, 7, 92, 13, 0, '', NULL, 0, 0, '', 1, '2024-03-02 02:12:17', 5, '2024-03-02 02:12:17', 5),
(754, 7, 93, 11, 0, '', NULL, 0, 0, '', 1, '2024-03-02 02:12:17', 5, '2024-03-02 02:12:17', 5),
(755, 7, 93, 12, 0, '', NULL, 0, 0, '', 1, '2024-03-02 02:12:17', 5, '2024-03-02 02:12:17', 5),
(756, 7, 93, 13, 0, '', NULL, 0, 0, '', 1, '2024-03-02 02:12:17', 5, '2024-03-02 02:12:17', 5),
(757, 7, 94, 11, 0, '', NULL, 0, 0, '', 1, '2024-03-02 02:12:17', 5, '2024-03-02 02:12:17', 5),
(758, 7, 94, 12, 0, '', NULL, 0, 0, '', 1, '2024-03-02 02:12:17', 5, '2024-03-02 02:12:17', 5),
(759, 7, 94, 13, 0, '', NULL, 0, 0, '', 1, '2024-03-02 02:12:17', 5, '2024-03-02 02:12:17', 5),
(760, 7, 95, 11, 0, '', NULL, 0, 0, '', 1, '2024-03-02 02:12:17', 5, '2024-03-02 02:12:17', 5),
(761, 7, 95, 12, 0, '', NULL, 0, 0, '', 1, '2024-03-02 02:12:17', 5, '2024-03-02 02:12:17', 5),
(762, 7, 95, 13, 0, '', NULL, 0, 0, '', 1, '2024-03-02 02:12:17', 5, '2024-03-02 02:12:17', 5),
(763, 7, 96, 11, 0, '', NULL, 0, 0, '', 1, '2024-03-02 02:12:17', 5, '2024-03-02 02:12:17', 5),
(764, 7, 96, 12, 0, '', NULL, 0, 0, '', 1, '2024-03-02 02:12:17', 5, '2024-03-02 02:12:17', 5),
(765, 7, 96, 13, 0, '', NULL, 0, 0, '', 1, '2024-03-02 02:12:17', 5, '2024-03-02 02:12:17', 5),
(766, 7, 97, 11, 0, '', NULL, 0, 0, '', 1, '2024-03-02 02:12:17', 5, '2024-03-02 02:12:17', 5),
(767, 7, 97, 12, 0, '', NULL, 0, 0, '', 1, '2024-03-02 02:12:17', 5, '2024-03-02 02:12:17', 5),
(768, 7, 97, 13, 0, '', NULL, 0, 0, '', 1, '2024-03-02 02:12:17', 5, '2024-03-02 02:12:17', 5),
(769, 7, 98, 11, 0, '', NULL, 0, 0, '', 1, '2024-03-02 02:12:17', 5, '2024-03-02 02:12:17', 5),
(770, 7, 98, 12, 0, '', NULL, 0, 0, '', 1, '2024-03-02 02:12:17', 5, '2024-03-02 02:12:17', 5),
(771, 7, 98, 13, 0, '', NULL, 0, 0, '', 1, '2024-03-02 02:12:17', 5, '2024-03-02 02:12:17', 5),
(772, 7, 99, 11, 0, '', NULL, 0, 0, '', 1, '2024-03-02 02:12:17', 5, '2024-03-02 02:12:17', 5),
(773, 7, 99, 12, 0, '', NULL, 0, 0, '', 1, '2024-03-02 02:12:17', 5, '2024-03-02 02:12:17', 5),
(774, 7, 99, 13, 0, '', NULL, 0, 0, '', 1, '2024-03-02 02:12:17', 5, '2024-03-02 02:12:17', 5),
(775, 7, 100, 11, 0, '', NULL, 0, 0, '', 1, '2024-03-02 02:12:17', 5, '2024-03-02 02:12:17', 5),
(776, 7, 100, 12, 0, '', NULL, 0, 0, '', 1, '2024-03-02 02:12:17', 5, '2024-03-02 02:12:17', 5),
(777, 7, 100, 13, 0, '', NULL, 0, 0, '', 1, '2024-03-02 02:12:17', 5, '2024-03-02 02:12:17', 5),
(778, 7, 101, 11, 0, '', NULL, 0, 0, '', 1, '2024-03-02 02:12:17', 5, '2024-03-02 02:12:17', 5),
(779, 7, 101, 12, 0, '', NULL, 0, 0, '', 1, '2024-03-02 02:12:17', 5, '2024-03-02 02:12:17', 5),
(780, 7, 101, 13, 0, '', NULL, 0, 0, '', 1, '2024-03-02 02:12:17', 5, '2024-03-02 02:12:17', 5),
(781, 7, 102, 11, 0, '', NULL, 0, 0, '', 1, '2024-03-02 02:12:17', 5, '2024-03-02 02:12:17', 5),
(782, 7, 102, 12, 0, '', NULL, 0, 0, '', 1, '2024-03-02 02:12:17', 5, '2024-03-02 02:12:17', 5),
(783, 7, 102, 13, 0, '', NULL, 0, 0, '', 1, '2024-03-02 02:12:17', 5, '2024-03-02 02:12:17', 5),
(784, 7, 103, 11, 0, '', NULL, 0, 0, '', 1, '2024-03-02 02:12:17', 5, '2024-03-02 02:12:17', 5),
(785, 7, 103, 12, 0, '', NULL, 0, 0, '', 1, '2024-03-02 02:12:17', 5, '2024-03-02 02:12:17', 5),
(786, 7, 103, 13, 0, '', NULL, 0, 0, '', 1, '2024-03-02 02:12:17', 5, '2024-03-02 02:12:17', 5);
...
CREATE TABLE `payements` (
  ...
  `typePayements` varchar(100) NOT NULL,
  `mois` varchar(45) DEFAULT NULL,
...
(126, 106, NULL, 4, 'mensuel', 'Mars', 00000300000, '2024-03-01 23:34:02', 1, '2024-03-01 23:34:02', 5, '2024-03-01 23:34:02', 5);
...
(279, 102, NULL, 2, 4, 1, '2023-11-23 23:54:55', 5, '2023-11-23 23:54:55', 5),
(280, 103, NULL, 2, 4, 1, '2023-12-16 16:22:32', 5, '2023-12-16 16:22:32', 5),
(281, 104, NULL, 2, 4, 1, '2023-12-17 00:31:39', 5, '2023-12-17 00:31:39', 5),
(282, 103, NULL, 19, 4, 1, '2024-02-05 22:50:43', 5, '2024-02-05 22:50:43', 5),
(283, 105, NULL, 20, 4, 1, '2024-02-29 22:21:57', 5, '2024-02-29 22:21:57', 5),
(284, 104, NULL, 20, 4, 1, '2024-03-01 23:44:39', 5, '2024-03-01 23:44:39', 5),
(285, 105, NULL, 2, 4, 1, '2024-03-01 23:49:41', 5, '2024-03-01 23:49:41', 5);
...
INSERT INTO `sessions` (`id`, `libelle`, `observations`, `estActif`, `creationDate`, `creationUserId`, `modifDate`, `modifUserId`) VALUES
(1, '2023-2024', NULL, 1, '2023-11-17 13:26:46', 1, '2023-11-17 13:26:46', 1),
(7, '2024-2025', NULL, 1, '2023-11-22 20:49:55', 1, '2023-11-22 20:49:55', 1),
(8, '2025-2026', NULL, 1, '2023-11-22 20:49:55', 1, '2023-11-22 20:49:55', 1);
...
CREATE TABLE `storysales` (
  ...
  `trancheId` int(11) DEFAULT NULL,
  `prestationId` int(11) NOT NULL,
  `typePayements` varchar(100) NOT NULL DEFAULT 'totalite',
  `mois` varchar(45) DEFAULT NULL,
...
(19, 106, NULL, 4, 'mensuel', 'Mars', 0000000000000000300000, '2024-03-01 23:27:44', 1, '2024-03-01 23:27:44', 5, '2024-03-01 23:27:44', 5),
(20, 106, NULL, 4, 'mensuel', 'Mars', 0000000000000000300000, '2024-03-01 23:29:04', 1, '2024-03-01 23:29:04', 5, '2024-03-01 23:29:04', 5),
(21, 106, NULL, 4, 'mensuel', 'Mars', 0000000000000000300000, '2024-03-01 23:34:02', 1, '2024-03-01 23:34:02', 5, '2024-03-01 23:34:02', 5);
...
CREATE TABLE `tickets` (
  ...
  `classeId` int(11) NOT NULL,
  `libelle` varchar(50) NOT NULL,
  `typeEvaluation` enum('Routinière','Officielle') DEFAULT 'Officielle',
  `observations` longtext DEFAULT NULL,
  `estActif` tinyint(4) NOT NULL DEFAULT 1,
  `creationDate` datetime NOT NULL,
  `creationUserId` int(11) NOT NULL,
...
INSERT INTO `tickets` (`id`, `sessionId`, `classeId`, `libelle`, `typeEvaluation`, `observations`, `estActif`, `creationDate`, `creationUserId`, `modifDate`, `modifUserId`) VALUES
(6, 1, 2, 'Examen controle 1', 'Officielle', '', 1, '2023-12-16 16:45:36', 5, '2023-12-16 16:45:36', 5),
(7, 1, 2, 'Examen controle 2', 'Officielle', '', 1, '2023-12-16 16:46:01', 5, '2023-12-16 16:46:01', 5),
(8, 1, 1, 'Evaluation 1', 'Officielle', '', 1, '2023-12-16 17:01:51', 5, '2023-12-16 17:01:51', 5),
(9, 1, 1, 'Evaluation 2', 'Officielle', '', 1, '2023-12-16 17:02:11', 5, '2023-12-16 17:02:11', 5),
(10, 1, 10, 'Evaluation du premier trimestre', 'Officielle', '', 1, '2024-02-01 22:40:45', 5, '2024-02-01 22:40:45', 5),
(11, 1, 11, 'Evaluation du premier trimestre', 'Officielle', '', 1, '2024-02-03 17:55:00', 5, '2024-02-03 17:55:00', 5),
(12, 1, 5, 'Evaluation du prémier semestre', 'Officielle', '', 1, '2024-02-03 18:15:16', 5, '2024-02-03 18:15:16', 5),
(13, 1, 5, 'Evaluation du second trimestre', 'Officielle', '', 1, '2024-02-03 18:15:40', 5, '2024-02-03 18:15:40', 5),
(14, 1, 11, 'Evaluation du second trimestre', 'Officielle', '', 1, '2024-02-18 15:06:20', 5, '2024-02-18 15:06:20', 5),
(15, 1, 3, 'Evaluation du premier trimestre', 'Officielle', '', 1, '2024-03-01 23:10:06', 5, '2024-03-01 23:13:57', 5);

-- --------------------------------------------------------

--
-- Structure de la table `ticket_matieres`
--
...
--
-- Déchargement des données de la table `ticket_matieres`
--

INSERT INTO `ticket_matieres` (`id`, `tiketId`, `matiereId`, `estActif`, `creationDate`, `creationUserId`, `modifDate`, `modifUserId`) VALUES
(6, 6, 11, 1, '2023-12-16 16:55:08', 5, '2023-12-16 16:55:08', 5),
(7, 6, 12, 1, '2023-12-16 16:55:16', 5, '2023-12-16 16:55:16', 5),
(8, 6, 13, 1, '2023-12-16 16:59:20', 5, '2023-12-16 16:59:20', 5),
(9, 6, 14, 1, '2023-12-16 16:59:32', 5, '2023-12-16 16:59:32', 5),
(10, 6, 15, 1, '2023-12-16 16:59:55', 5, '2023-12-16 16:59:55', 5),
(11, 6, 16, 1, '2023-12-16 17:00:06', 5, '2023-12-16 17:00:06', 5),
(12, 6, 17, 1, '2023-12-16 17:00:20', 5, '2023-12-16 17:00:20', 5),
(13, 8, 6, 1, '2023-12-16 17:02:28', 5, '2023-12-16 17:02:28', 5),
(14, 8, 7, 1, '2023-12-16 17:02:44', 5, '2023-12-16 17:02:44', 5),
(15, 8, 8, 1, '2023-12-16 17:02:53', 5, '2023-12-16 17:02:53', 5),
(16, 8, 9, 1, '2023-12-16 17:03:06', 5, '2023-12-16 17:03:06', 5),
(17, 8, 10, 1, '2023-12-16 17:03:14', 5, '2023-12-16 17:03:14', 5),
(18, 9, 7, 1, '2023-12-16 17:03:29', 5, '2023-12-16 17:03:29', 5),
(19, 9, 8, 1, '2023-12-16 17:03:37', 5, '2023-12-16 17:03:37', 5),
(20, 9, 9, 1, '2023-12-16 17:03:47', 5, '2023-12-16 17:03:47', 5),
(21, 10, 18, 1, '2024-02-01 22:41:06', 5, '2024-02-01 22:41:06', 5),
(22, 10, 19, 1, '2024-02-01 22:41:20', 5, '2024-02-01 22:41:20', 5),
...
(31, 7, 11, 1, '2024-03-02 02:11:04', 5, '2024-03-02 02:11:04', 5),
(32, 7, 12, 1, '2024-03-02 02:11:16', 5, '2024-03-02 02:11:16', 5),
(33, 7, 13, 1, '2024-03-02 02:11:26', 5, '2024-03-02 02:11:26', 5);
...
(71, 33, 18, 1, '2024-02-05 21:06:10', 5, '2024-02-05 21:06:10', 5),
(72, 33, 19, 1, '2024-02-05 21:06:21', 5, '2024-02-05 21:06:21', 5),
(73, 34, 18, 1, '2024-02-05 22:09:04', 5, '2024-02-05 22:09:04', 5),
(74, 34, 19, 1, '2024-02-05 22:09:11', 5, '2024-02-05 22:09:11', 5),
(75, 35, 18, 1, '2024-02-05 22:10:12', 5, '2024-02-05 22:10:12', 5),
(76, 35, 19, 1, '2024-02-05 22:10:20', 5, '2024-02-05 22:10:20', 5),
(77, 36, 18, 1, '2024-02-05 22:10:56', 5, '2024-02-05 22:10:56', 5),
(78, 36, 19, 1, '2024-02-05 22:11:02', 5, '2024-02-05 22:11:02', 5),
(79, 37, 18, 1, '2024-02-05 22:11:23', 5, '2024-02-05 22:11:23', 5),
(80, 37, 19, 1, '2024-02-05 22:11:29', 5, '2024-02-05 22:11:29', 5),
(81, 38, 18, 1, '2024-02-05 22:12:34', 5, '2024-02-05 22:12:34', 5),
(82, 38, 19, 1, '2024-02-05 22:12:42', 5, '2024-02-05 22:12:42', 5),
(83, 39, 18, 1, '2024-02-05 22:13:25', 5, '2024-02-05 22:13:25', 5),
(84, 39, 19, 1, '2024-02-05 22:13:31', 5, '2024-02-05 22:13:31', 5);
...
(5, 2, 'Bangoura', 'Fatoumata', NULL, '621196376', '99857914190f407368f5396b0bc84606', 50, 1, '2023-10-15 08:04:28', 1, '2024-01-28 20:09:34', 5),
...
(34, 2, 'Sow', 'Thierno Ahmadou', NULL, '620311515', '5f4dcc3b5aa765d61d8327deb882cf99', 88, 1, '2024-02-05 22:09:04', 5, '2024-02-05 22:09:04', 5),
(35, 2, 'Soumah', 'Fatoumata', NULL, '622121314', '5f4dcc3b5aa765d61d8327deb882cf99', 49, 1, '2024-02-05 22:10:12', 5, '2024-02-05 22:10:12', 5),
(36, 2, 'Yattara', 'Amara', NULL, '625001234', '5f4dcc3b5aa765d61d8327deb882cf99', 52, 1, '2024-02-05 22:10:56', 5, '2024-02-05 22:10:56', 5),
(37, 2, 'oularé', 'Fatoumata simba', NULL, '621196371', '5f4dcc3b5aa765d61d8327deb882cf99', 86, 1, '2024-02-05 22:11:23', 5, '2024-02-05 22:11:23', 5),
(38, 2, 'Soumah', 'Abdoulaye Foté', NULL, '621151515', '5f4dcc3b5aa765d61d8327deb882cf99', 89, 1, '2024-02-05 22:12:34', 5, '2024-02-05 22:12:34', 5),
(39, 2, 'sidibé', 'Souleymane', NULL, '627001532', '5f4dcc3b5aa765d61d8327deb882cf99', 37, 1, '2024-02-05 22:13:25', 5, '2024-02-05 22:13:25', 5);
...
ALTER TABLE `eleves`
  ...
  ADD KEY `fk_eleves_sessionId_idx` (`sessionId`);

--
-- Index pour la table `enseignantclasses`
--
...
ALTER TABLE `matieres`
  ADD PRIMARY KEY (`id`),
  ADD KEY `fk_matieres_classeId_idx` (`classeId`);
...
ALTER TABLE `notes`
  ADD PRIMARY KEY (`id`),
  ADD KEY `fk_matieres_eleveId_idx` (`eleveId`),
  ADD KEY `fk_matieres_eleveId_idx1` (`matiereId`),
  ADD KEY `fk_matieres_ticketId_idx` (`ticketId`);
...
ALTER TABLE `tickets`
  ADD PRIMARY KEY (`id`),
  ADD KEY `fk_tickets_sessionId_idx` (`sessionId`),
  ADD KEY `fk_tickets_classeId_idx` (`classeId`);

--
-- Index pour la table `ticket_matieres`
--
ALTER TABLE `ticket_matieres`
  ADD PRIMARY KEY (`id`),
  ADD KEY `fk_ticket_matiere_ticketId_idx` (`tiketId`),
  ADD KEY `fk_ticket_matiere_matiere_idx` (`matiereId`);
...
--
-- AUTO_INCREMENT pour la table `enseignantclasses`
--
ALTER TABLE `enseignantclasses`
...
--
-- AUTO_INCREMENT pour la table `ticket_matieres`
--
ALTER TABLE `ticket_matieres`
...
ALTER TABLE `eleves`
  ...
  ADD CONSTRAINT `fk_eleves_sessionId` FOREIGN KEY (`sessionId`) REFERENCES `sessions` (`id`) ON UPDATE CASCADE;

--
-- Contraintes pour la table `enseignantclasses`
--
ALTER TABLE `enseignantclasses`
...
--
ALTER TABLE `matieres`
  ADD CONSTRAINT `fk_matieres_classeId` FOREIGN KEY (`classeId`) REFERENCES `classses` (`id`) ON UPDATE CASCADE;

--
-- Contraintes pour la table `notes`
--
ALTER TABLE `notes`
  ADD CONSTRAINT `fk_notes_eleveId` FOREIGN KEY (`eleveId`) REFERENCES `eleves` (`id`) ON UPDATE CASCADE,
  ADD CONSTRAINT `fk_notes_matiereId` FOREIGN KEY (`matiereId`) REFERENCES `matieres` (`id`) ON UPDATE CASCADE,
  ADD CONSTRAINT `fk_notes_ticketId` FOREIGN KEY (`ticketId`) REFERENCES `tickets` (`id`) ON UPDATE CASCADE;
...
--
ALTER TABLE `tickets`
  ADD CONSTRAINT `fk_tickets_classeId` FOREIGN KEY (`classeId`) REFERENCES `classses` (`id`) ON UPDATE CASCADE,
  ADD CONSTRAINT `fk_tickets_sessionId` FOREIGN KEY (`sessionId`) REFERENCES `sessions` (`id`) ON UPDATE CASCADE;

--
-- Contraintes pour la table `ticket_matieres`
--
ALTER TABLE `ticket_matieres`
  ADD CONSTRAINT `fk_ticket_matieres_matiere` FOREIGN KEY (`matiereId`) REFERENCES `matieres` (`id`) ON UPDATE CASCADE,
  ADD CONSTRAINT `fk_ticket_matieres_ticketId` FOREIGN KEY (`tiketId`) REFERENCES `tickets` (`id`) ON UPDATE CASCADE;
