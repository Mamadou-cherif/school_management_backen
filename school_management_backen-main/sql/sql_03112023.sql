...
--
-- Hôte : 127.0.0.1
-- Généré le : ven. 03 nov. 2023 à 01:28
...
(1, 88, NULL, 4, 'mensuel', 'Octobre', 0000000000000000300000, '2023-11-03 00:26:01', 1, '2023-11-03 00:26:01', 5, '2023-11-03 00:26:01', 5);
...
CREATE TABLE `schoolpayementmanager1`.`storysales` (
  `id` INT(11) NOT NULL AUTO_INCREMENT,
  `eleveId` INT(11) NOT NULL,
  `trancheId` INT(11) NULL DEFAULT NULL,
  `prestationId` INT(11) NULL DEFAULT NULL,
  `typePayements` ENUM('partiel', 'totalite', 'modulaire') NOT NULL DEFAULT 'totalite',
  `mois` VARCHAR(45) NULL DEFAULT NULL,
  `prix` DOUBLE UNSIGNED ZEROFILL NOT NULL,
  `datePayement` VARCHAR(45) NULL DEFAULT NULL,
  `estActif` TINYINT(4) NOT NULL DEFAULT 1,
  `creationDate` DATETIME NOT NULL,
  `creationUserId` INT(11) NOT NULL,
  `modifDate` DATETIME NOT NULL,
  `modifUserId` INT(11) NOT NULL,
  INDEX `fk_storySales_eleveId_idx` (`eleveId` ASC),
  INDEX `fk_storySales_prestationId_idx` (`prestationId` ASC),
  INDEX `fk_storySales_trancheId_idx` (`trancheId` ASC),
  PRIMARY KEY (`id`),
  CONSTRAINT `fk_storySales_eleveId`
    FOREIGN KEY (`eleveId`)
    REFERENCES `schoolpayementmanager1`.`eleves` (`id`)
    ON DELETE RESTRICT
    ON UPDATE CASCADE,
  CONSTRAINT `fk_storySales_prestationId`
    FOREIGN KEY (`prestationId`)
    REFERENCES `schoolpayementmanager1`.`prestations` (`id`)
    ON DELETE RESTRICT
    ON UPDATE CASCADE,
  CONSTRAINT `fk_storySales_trancheId`
    FOREIGN KEY (`trancheId`)
    REFERENCES `schoolpayementmanager1`.`tranches` (`id`)
    ON DELETE RESTRICT
    ON UPDATE CASCADE);
