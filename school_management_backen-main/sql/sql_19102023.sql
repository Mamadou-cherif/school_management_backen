...
--
-- Hôte : 127.0.0.1
-- Généré le : ven. 20 oct. 2023 à 01:46
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `payements_insert` (`eleveId` INT(11), `trancheId` INT(11), `prestationId` INT(11), `typePayements` ENUM('partiel','totalite'), `mois` VARCHAR(45), `prix` DOUBLE UNSIGNED ZEROFILL, `datePayement` VARCHAR(45), `creationUserId` INT)   BEGIN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `prestations_insert` (`libelle` VARCHAR(45), `modePaiement` ENUM('mensuel','modulaire'), `uniteDuree` ENUM('mois','semaine','jour','heure'), `estActif` TINYINT(4), `creationUserId` INT)   BEGIN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `prestations_selectBy` (`id` INT(11), `libelle` VARCHAR(45), `modePaiement` ENUM('mensuel','modulaire'), `uniteDuree` ENUM('mois','semaine','jour','heure'), `estActif` TINYINT(4), `creationDate` DATETIME, `creationUserId` INT(11), `modifDate` DATETIME, `modifUserId` INT(11), `duree` INT(11))   BEGIN
 DECLARE requeteSql varchar(500); 
	 SET @requeteSql :=   
		 'SELECT
			prestations.id,
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `prestations_update` (`id` INT, `libelle` VARCHAR(45), `modePaiement` ENUM('mensuel','modulaire'), `uniteDuree` ENUM('mois','semaine','jour','heure'), `estActif` TINYINT(4), `modifDate` DATETIME, `modifUserId` INT)   BEGIN
	 UPDATE prestations
		 SET 
			libelle = libelle, 
...
CREATE TABLE `payements` (
  ...
  `typePayements` enum('partiel','totalite') NOT NULL DEFAULT 'totalite',
...
CREATE TABLE `prestations` (
  `id` int(11) NOT NULL,
  `libelle` varchar(45) NOT NULL,
  `modePaiement` enum('mensuel','modulaire') NOT NULL,
  `duree` int(11) DEFAULT NULL,
  `uniteDuree` enum('mois','semaine','jour','heure') NOT NULL,
  `estActif` tinyint(4) NOT NULL DEFAULT 1,
  `creationDate` datetime NOT NULL,
  `creationUserId` int(11) NOT NULL,
  `modifDate` datetime NOT NULL,
...
ALTER TABLE `fiche_renseignements`
  ...
  ADD CONSTRAINT `fk_fiche_renseigments_prestationId` FOREIGN KEY (`prestationId`) REFERENCES `prestations` (`id`) ON UPDATE CASCADE;
